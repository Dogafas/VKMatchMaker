...
def save_search_results(user_id, result_user_id, first_name, last_name, profile_url):
...
def get_search_result_by_id(search_result_id): # <-ЗДЕСЬ ИЗМЕНЕНИЯ: (добавлена новая функция)
    """Получает результат поиска по его ID."""
    logging.info(f"Начало get_search_result_by_id с search_result_id: {search_result_id}")
    search_result = session.scalar(
        select(Search_Result).where(Search_Result.id == search_result_id)
    )
    if not search_result:
        logging.info(f"Результат поиска с search_result_id: {search_result_id} не найден")
        return None
    logging.info(f"Результат поиска с search_result_id: {search_result_id} получен: {search_result}")
