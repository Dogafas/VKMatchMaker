...
def _handle_yes_button(user_id):  # <-ЗДЕСЬ ИЗМЕНЕНИЯ (обработчик нажатия кнопки "Да")
...
def _handle_no_button(user_id):  # <-ЗДЕСЬ ИЗМЕНЕНИЯ (обработчик нажатия кнопки "Нет")
...
def _handle_sex_button(user_id, sex):  # <-ЗДЕСЬ ИЗМЕНЕНИЯ (обработчик нажатия кнопок "Мужской" и "Женский")
...
def _handle_age_button(user_id, age):  # <-ЗДЕСЬ ИЗМЕНЕНИЯ (обработчик нажатия кнопок с возрастом)
...
def _show_next_user(user_id):  # <-ЗДЕСЬ ИЗМЕНЕНИЯ (вывод следующего пользователя)
...
def _handle_search_button(user_id):  # <-ЗДЕСЬ ИЗМЕНЕНИЯ (обработчик кнопки "Далее")
    current_index = get_user_index(user_id) + 1
    change_user_index(user_id, current_index)

    users = search_users(user_id, offset=current_index, count=1)  # <-ЗДЕСЬ ИЗМЕНЕНИЯ (ищем открытый профиль)
    if users:  # <-ЗДЕСЬ ИЗМЕНЕНИЯ (проверяем что нашли)
        ...
        send_message_from_group(
            user_id,
            "Не удалось найти следующего пользователя",
            keyboard=get_next_prev_keyboard(),
        )


def _handle_prev_button(user_id):  # <-ЗДЕСЬ ИЗМЕНЕНИЯ (обработчик кнопки "Назад")
    current_index = get_user_index(user_id)
    if current_index > 0:
        current_index -= 1
        change_user_index(user_id, current_index)
        search_results = get_search_results(user_id)
        if search_results and len(search_results) > current_index:
            result_user_id = search_results[current_index].result_user_id
            found_user_info = get_user_info(
                result_user_id
            )
            message = (
...
def _handle_start_command(user_id):  # <-ЗДЕСЬ ИЗМЕНЕНИЯ (обработчик команды "Начать")
...
def _handle_favorite_button(user_id):  # <-ЗДЕСЬ ИЗМЕНЕНИЯ (обработчик кнопки "Избранное")
    current_index = get_user_index(user_id)
    search_results = get_search_results(user_id)
    if search_results and len(search_results) > current_index:
        result_user_id = search_results[current_index].result_user_id
        set_favorite(user_id, result_user_id)
        send_message_from_group(
            user_id, "Пользователь добавлен в избранное!", keyboard=get_next_prev_keyboard()
        )
    else:
        send_message_from_group(
            user_id, "Не удалось добавить пользователя в избранное.", keyboard=get_next_prev_keyboard()
        )


def _handle_blacklist_button(user_id):  # <-ЗДЕСЬ ИЗМЕНЕНИЯ (обработчик кнопки "Черный список")
    current_index = get_user_index(user_id)
    search_results = get_search_results(user_id)
    if search_results and len(search_results) > current_index:
        result_user_id = search_results[current_index].result_user_id
        set_blacklist(user_id, result_user_id)
        send_message_from_group(
            user_id, "Пользователь добавлен в черный список!", keyboard=get_next_prev_keyboard()
        )
    else:
        send_message_from_group(
            user_id, "Не удалось добавить пользователя в черный список.", keyboard=get_next_prev_keyboard()
        )
        ...
        if payload:
            try:
                ...
                button_handlers = {
                    ...
                    "favorite": _handle_favorite_button,  # <-ЗДЕСЬ ИЗМЕНЕНИЯ (добавили обработчик для кнопки "Избранное")
                    "blacklist": _handle_blacklist_button,  # <-ЗДЕСЬ ИЗМЕНЕНИЯ (добавили обработчик для кнопки "Черный список")
                }
            ...
            _handle_start_command(user_id)  # <-ЗДЕСЬ ИЗМЕНЕНИЯ (вызываем функцию обработчик команды "Начать")
