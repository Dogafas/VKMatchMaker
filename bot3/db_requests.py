...
def save_search_results(user_id, result_user_id, search_params_id):
    logging.info(f"Начало save_search_results с user_id: {user_id}, result_user_id: {result_user_id}, search_params_id: {search_params_id}")  # <-ЗДЕСЬ ИЗМЕНЕНИЯ ("Добавили логирование начала функции")
...
def get_user_index(user_id):
    ...
    logging.info(f"Текущий индекс пользователя {user_id}: {index.current_index}")  
    return index.current_index


def change_user_index(user_id, index):
    logging.info(f"Начало change_user_index с user_id: {user_id}, index: {index}")  
    change_index = session.scalar(select(User).where(User.user_id == user_id))

    change_index.current_index = index
    session.commit()
    logging.info(f"Индекс пользователя {user_id} изменен на: {index}")  

def set_favorite(user_id, result_user_id):
    """
    Устанавливает флаг favorites в True для указанного результата поиска.
    """
    logging.info(f"Начало set_favorite с user_id: {user_id}, result_user_id: {result_user_id}")
    ...
    logging.info(f"Флаг favorites установлен в True для user_id: {user_id}, result_user_id: {result_user_id}")
    return "Favorite flag set to True"


def set_blacklist(user_id, result_user_id):
    """
    Устанавливает флаг blacklist в True для указанного результата поиска.
    """
    logging.info(f"Начало set_blacklist с user_id: {user_id}, result_user_id: {result_user_id}")
    ...
    logging.info(f"Флаг blacklist установлен в True для user_id: {user_id}, result_user_id: {result_user_id}")
