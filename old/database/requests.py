from bot.session import async_session, engine
from bot.models import User, User_Data, Blacklist, Favourite, SearchUsers, UserParams
...
async def search_users_data(vk_id, search_user_id):
    async with engine.begin() as conn:
        async with async_session(bind=conn) as session:
            search_user = await session.scalar(select(SearchUsers).where(vk_id=vk_id, search_user_id=search_user_id))

            if not search_user:
                await session.add(SearchUsers(vk_id=vk_id, search_user_id=search_user_id))
                return 'User added to search list'
            
            return 'User already in search list'

async def set_user_params(vk_id, sex):
    async with engine.begin() as conn:
        async with async_session(bind=conn) as session:
            user_params = await session.scalar(select(UserParams).where(vk_id=vk_id))

            if not user_params:
                await session.add(UserParams(vk_id=vk_id, sex=sex))