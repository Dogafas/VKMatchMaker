...
#         try:
#             with conn.cursor() as cur:
#                 cur.execute("SELECT id, search_offset FROM users WHERE vk_user_id = %s", (vk_user_id,))
#                 user_data = cur.fetchone()
#                 if user_data:
#                      logging.info(f"id пользователя {vk_user_id} получен из БД")
#                      return {"id":user_data[0], "offset":user_data[1]}
#                 else:
...
def get_user_id(vk_user_id):
    ...
    if conn:
        try:
            with conn.cursor() as cur:
                cur.execute("SELECT id, search_offset, last_viewed_result_id FROM users WHERE vk_user_id = %s", (vk_user_id,)) # <-ЗДЕСЬ ИЗМЕНЕНИЯ (добавили last_viewed_result_id)
                user_data = cur.fetchone()
                if user_data:
                    logging.info(f"id пользователя {vk_user_id} получен из БД")
                    return {"id":user_data[0], "offset":user_data[1], "last_viewed_result_id":user_data[2]} # <-ЗДЕСЬ ИЗМЕНЕНИЯ (добавили last_viewed_result_id)
                else:
                    logging.info(f"id пользователя {vk_user_id} в БД не найден")
                    return None
        except psycopg2.Error as e:
...
def update_user_last_viewed_result_id(vk_user_id, last_viewed_result_id):  # <-ЗДЕСЬ ИЗМЕНЕНИЯ (функция обновления last_viewed_result_id)
    conn = get_db_connection()
    if conn:
        try:
            with conn.cursor() as cur:
...
def get_max_search_result_id(user_id):  # <-ЗДЕСЬ ИЗМЕНЕНИЯ (функция получения максимального id)
    conn = get_db_connection()
    if conn:
        try:
            with conn.cursor() as cur:
