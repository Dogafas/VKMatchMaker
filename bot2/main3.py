...
from database import save_user, get_user_id, save_search_result, get_db_connection, save_blacklisted_user, is_user_blacklisted, is_search_result_exists, update_user_offset, update_user_last_viewed_result_id, get_max_search_result_id
            ...
            try:
                ...
                if button == "yes":
                    ...
                    if users:
                        ...
                        new_user = None
                        if not users: # <-ЗДЕСЬ ИЗМЕНЕНИЯ (проверка на пустой список)
                            offset += 10 # <-ЗДЕСЬ ИЗМЕНЕНИЯ (увеличиваем смещение на 10 если нет результатов)
                            update_user_offset(user_id, offset)
                            logging.info(f"Нет новых анкет, увеличено смещение на 10, новый offset: {offset}")
                                ...
                                if not is_search_result_exists(user_db_id, user["id"]):
                                    ...
                                    if found_user_info:
                                        ...
                                        break
                                         ...
                                         offset += 1
                                         update_user_offset(user_id, offset)
                                         logging.info(f"Увеличен offset для пользователя {user_id}, новый offset: {offset}")
                            if new_user:
                                ...
                                if found_user_info:
                                    ...
                                    update_user_offset(user_id, offset)
                                    update_user_last_viewed_result_id(user_id, None)
